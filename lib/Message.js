...
module.exports = (rem, messageRaw) => {
    ...
    message.sender = message.isGroupMsg ? message.key.participant.split(':')[0] : message.fromMe ? rem.user.jid : message.from.split(':')[0]
    ...
    if(message.quotedMsg) {
        ...
        message.quotedMsg.sender = message.message[message.type].contextInfo ? message.message[message.type].contextInfo.participant || rem.user.jid : message.message.contextInfo.participant || rem.user.jid
        message.quotedMsg.fromMe = message.quotedMsg.sender === (rem.user && rem.user.jid)
        message.quotedMsg.pushname = message.quotedMsg.fromMe ? rem.user.name : rem.contacts(message.quotedMsg.sender)
        ...
        message.quotedMsgObj = async () => {
            if(!message.quotedMsg.id) return false
            let msg = await rem.loadMessage(message.quotedMsg.from, message.quotedMsg.id)
            return serializeMsgObj(rem, msg)
        }
